...
class TestPasswordSecurity:
    ...
    def scanner_results(self):
        ...
        # Get repository root - adjust parent calls based on your file structure
        # If test_passwords.py is at: tests/test_cases/utils/test_scripts/test_passwords.py
        # Then: parent(1)=test_scripts, parent(2)=utils, parent(3)=test_cases, 
        #       parent(4)=tests, parent(5)=repo_root
        
        # Option 1: Auto-detect by finding .git directory
        current = Path(__file__).parent
        while current != current.parent:
            if (current / '.git').exists() or (current / '.gitlab-ci.yml').exists():
                repo_path = current
                break
            current = current.parent
        else:
            # Fallback: Go up 4 levels (adjust as needed)
